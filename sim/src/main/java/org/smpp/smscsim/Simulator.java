...
public class Simulator {
	...
	//BlockingQueue<ClientMessage> clientQueue = new ArrayBlockingQueue<>(10);
	ArrayList<ClientMessage> clientQueue = new ArrayList<>();
...
//		debug.deactivate(DSIMD2);
		...
		while (keepRunning) {
			...
			switch (optionInt) {
				...
				case 4 :
					...
					break;
				case 5 :
					{ArrayList<ClientMessage> mm=messageList();
					for(ClientMessage m:mm)
	 ...
	 */
	public ArrayList<ClientMessage> messageList() {
		
		if (smscListener != null) {
		clientQueue=messageStore.print();
		} else {
			System.out.println("You must start listener first.");
		}
//		ClientMessage m=new ClientMessage();
//		m.setShortMessage("exit");
//		try {
//			clientQueue.put(m);
...
//	public void dequeueMessageQueue(BlockingQueue<ClientMessage> clientQueue){
//		c
//		
//	}
		...
		if (smscListener != null) {
			synchronized (processors) {
				...
				if (procCount > 0) {
					...
					for (int i = 0; i < procCount; i++) {
						proc = (SimulatorPDUProcessor) processors.get(i);
						System.out.print("client:"+proc.getSystemId());
						clients.add(proc.getSystemId());
						if (!proc.isActive()) {
