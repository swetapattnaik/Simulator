...
public class ShortMessageStore {
	 ...
	 */
	public synchronized ArrayList<ClientMessage> print() {
		//ArrayList<ClientMessage> mm=new ArrayList<>();ArrayList
		//BlockingQueue<ClientMessage> mesgQueue = new ArrayBlockingQueue<>(10);;
		ArrayList<ClientMessage> mesgQueue = new ArrayList<>();
		if (messages.size() != 0) {
			...
			while (keys.hasMoreElements()) {
				...
				ClientMessage message=printMessage(key, sMV);
				if(!mesgQueue.contains(message))
					mesgQueue.add(message);
	 ...
	 */
	private ClientMessage printMessage(Object key, ShortMessageValue sMV) {
